...
%    boxplot_data = [];
%    plot_data_struct = sigmoid_struct(strcmp({sigmoid_struct.stim_idx}, stim_type_list{stim_type}));
%    for stim_set = 1:length(plot_data_struct)
%        for muscle = 1:length(filled_muscle)
%            boxplot_data(stim_set,muscle) = plot_data_struct(stim_set).(filled_muscle{muscle})(sig_param);
%        end
%    end
%    
%    figure
%    hold on
% %    grid on
%    boxplot(boxplot_data,[1:5],'orientation', 'horizontal')
%    
%    for r = 1:size(boxplot_data,1)
%        scatter(boxplot_data(r,:),1:5,'filled')
%    end
%    ylim([0 6])
%    xlim([-500 3000])
% %    xlim([0 8000])
% %    xlim([0 max(max(boxplot_data))+500])
% 
%    title(stim_type_list{stim_type},'Interpreter','none')
%    yticks(1:6)
%    yticklabels(filled_muscle)
%    set(gca,'TickLabelInterpreter','none')
%    ylabel('Muscle')
%    xlabel('Activation Threshold (uA)')
%    
% %    figure
% %    notBoxPlot(box_plot_data,1:5)
% %    title(stim_type_list{stim_type},'Interpreter','none')
%    
% end
